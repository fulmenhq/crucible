...
async function main(): Promise<number> {
  ...
  if (exitCode === 0) {
    ...
    if (driftDetected) {
      ...
      console.error("  • Review changes: git diff fulencode/ lang/python/src/crucible/fulencode/ lang/typescript/src/fulencode/");
...
function regenerateTypes(): void {
  ...
  try {
    ...
    execSync(
      `bun run ${ROOT}/scripts/codegen/generate-fulencode-types.ts --lang python --format`,
      {
        cwd: ROOT,
        stdio: "pipe",
      }
    ...
    execSync(
      `bun run ${ROOT}/scripts/codegen/generate-fulencode-types.ts --lang go --format`,
      {
        cwd: ROOT,
        stdio: "pipe",
      }
