...
This release adds SIGKILL as a first-class signal entry and GNU timeout/shell exit codes (124-127) to support process supervision semantics in rsfulmen's sysprims foundation stage. It also fixes a schema/data drift issue in the sync-keys configuration and aligns CI lint tooling (yamllint) with goneat's formatting behavior.
...
#### CI Lint Configuration Alignment

Added `.yamllint.yaml` to align yamllint rules with goneat's YAML formatting behavior.

**Problem**: goneat's YAML formatter does not add `---` document-start markers to single-document files, but yamllint's default rules warn about missing markers. This caused false-positive "exit status 2" reports in assess hooks.

**Solution**: Added `.yamllint.yaml` configuration:

- `document-start: disable` - Aligns with goneat output and GitHub Actions conventions
- `line-length: max: 120, level: warning` - Matches goneat defaults
- `truthy: check-keys: false` - Allows common YAML patterns

**Verification**: Confirmed `---` is optional for GitHub Actions (passes actionlint) and is omitted by major OSS projects (e.g., rust-lang/rust).

## Commits

```
5e3d59d fix(ci): add yamllint config to align with goneat formatting
