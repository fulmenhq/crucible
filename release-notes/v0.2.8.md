...
Documentation and process improvement release establishing the CDRL (Clone → Degit → Refit → Launch) template standard system and fixing exit codes snapshot staleness. This release provides comprehensive guidance for template customization workflows, adds three forge template categories to the taxonomy, and enhances the build pipeline to prevent YAML/JSON drift.
...
- ✅ Prevents future YAML/snapshot drift
- ✅ Automated in standard workflow (`make sync` before commits)
- ✅ Standalone target for manual invocation
- ✅ Clear build pipeline documentation
...
```
❌ Found hardcoded 'groningen' references (see above)
...
2. Edit `.fulmen/app.yaml` with test identity (breed → `testapp`)
...
- ✅ Version consistency: Taxonomy 0.2.8 = repository VERSION
- ✅ Exit code snapshot: Regenerated with correct signal descriptions
- ✅ CDRL documentation: Complete cross-linking verified
- ✅ SSOT sync: Documentation synced to Python + TypeScript wrappers
...
- `config/taxonomy/metrics.yaml` - Version update (0.2.7 → 0.2.8)
...
- ✅ CDRL architectural standard published
- ✅ CDRL workflow guide published
- ✅ Required documentation outline specified
- ✅ Makefile standard Annex B published
- ✅ Forge standards updated with CDRL compliance
- ✅ Three forge template categories added to taxonomy
- ✅ Exit codes snapshot staleness fixed
- ✅ Two-stage sync pipeline implemented

**Forge Templates (Next)**:

- 🔜 groningen: Adopt CDRL v0.2.8 standard (add docs, validation targets)
- 🔜 percheron: Adopt CDRL v0.2.8 standard (add docs, validation targets)
- 🔜 aurora: Adopt CDRL v0.2.8 standard at launch
