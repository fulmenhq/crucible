...
Major feature release adding web styling infrastructure, server management orchestration, protocol-centric schema taxonomy, and codex configuration standards. This release establishes the foundation for consistent server operations, web template styling, and documentation site configuration across the Fulmen ecosystem.
...
- **Codex Configuration**: Standardized configuration for documentation sites and schema registries
...
**Migration**: `schemas/api/http/` → `schemas/protocol/http/`
...
- Updated all $id URLs: `api/http` → `protocol/http`
...
- Moved: `docs/standards/api/` → `docs/standards/protocol/`
...
**Examples**: `examples/api/http/` → `examples/protocol/http/`
...
### 4. Codex Configuration Standard

Standardized configuration schema for Fulmen codex templates (documentation sites, schema registries, knowledge hubs).
...
- `docs/standards/repository-category/codex/config-standard.md` - Comprehensive configuration standard
- `docs/standards/repository-category/README.md` - Overview of category-specific standards
...
```json
{
  ...
  "extensions": [],
  "features": {
    "search": { "provider": "starlight-default" },
    "i18n": { "enabled": false },
...
- **Controlled Enum**: Extension names controlled via schema enum
- **Governance Process**: Propose → review → add to schema → document → release
- **Pluggable Features**: schema-registry extension, future extensions (api-explorer, etc.)
- **Validation**: JSON Schema ensures only approved extensions are activated

**Repository-Category Organization**:

- **Parallel Structure**:
  - Schemas: `schemas/config/repository-category/{category}/v1.0.0/`
  - Docs: `docs/standards/repository-category/{category}/`
- **Scalability**: Ready for future categories (workhorse, helper-library)
- **Clear Taxonomy**: Category-specific standards separate from ecosystem-wide standards

**Template Integration**:

- Read codex-config.json during CRDL refit phase
- Apply project metadata to site config
- Enable/disable extensions based on configuration
- Configure features (search, i18n, analytics)
...
- Read and validate codex-config.json during CRDL refit
- Apply project metadata and extension configuration
- Customize features (search, i18n, analytics)
...
- Update imports: `schemas/api/http/` → `schemas/protocol/http/`
...
- ✅ Lint: All languages passed (Biome, Ruff, golangci-lint)
- ✅ Format: All files formatted (goneat)
- ✅ Typecheck: TypeScript passed
- ✅ Sync: All assets synced to language wrappers
- ✅ Exit code generation: Parity verified across all languages
...
- `schemas/config/repository-category/codex/v1.0.0/codex-config.schema.json`
...
- `docs/standards/repository-category/` - Repository-category-specific standards
  - `docs/standards/repository-category/README.md` - Category standards overview
  - `docs/standards/repository-category/codex/config-standard.md` - Codex configuration standard
...
- `examples/config/repository-category/codex/v1.0.0/codex-config.example.json`
...
- `schemas/api/http/` → `schemas/protocol/http/` (moved, not deleted)

**Documentation**:

- `docs/standards/api/` → `docs/standards/protocol/` (moved, not deleted)

**Examples**:

- `examples/api/http/` → `examples/protocol/http/` (moved, not deleted)
...
- ✅ Web styling schemas and config (SSOT established)
- ✅ Server management schemas and config (SSOT established)
- ✅ Protocol namespace taxonomy (schemas migrated)
- ✅ Codex configuration standard (schema, docs, examples)
- ✅ Repository-category infrastructure (parallel schema/doc structure)
- ✅ Architecture guides and module specs (documentation complete)
- ✅ Makefile targets annex (standards documented)

**v0.2.4+ (Planned)**:

- 🔜 Helper library implementations (tsfulmen, pyfulmen, gofulmen server modules)
- 🔜 Forge Codex Pulsar integration (web styling, server orchestration, codex config)
- 🔜 Additional protocol schemas (startup, liveness, readiness, metrics endpoints)
...
- [Codex Configuration Standard](../docs/standards/repository-category/codex/config-standard.md)
- [Repository Category Standards](../docs/standards/repository-category/README.md)
