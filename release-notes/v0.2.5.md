...
- **Windows**: Console event mappings (SIGTERM→CTRL_CLOSE_EVENT, SIGINT→CTRL_C_EVENT, SIGQUIT→CTRL_BREAK_EVENT)
...
- Cleanup action ordering (close_connections → flush_buffers → remove_pid_file → log_shutdown)
...
- **SIGUSR1**: 148 → 138 (128 + 10, Linux signal number)
- **SIGUSR2**: 144 → 140 (128 + 12, Linux signal number)
...
- ✅ **gofulmen**: Catalog metadata, log/telemetry contract, admin endpoint spec, testing guidance, operational docs
- ✅ **pyfulmen**: Signal introspection, telemetry standardization, HTTP contract, config reload parity, CI coverage
- ✅ **tsfulmen**: Windows fallback, admin surface spec, testing strategy, catalog patterns, operational guidance
...
- ✅ Go tests: All passed
- ✅ TypeScript tests: All passed (snapshots updated for exit code corrections)
- ✅ Python tests: All passed
- ✅ Schema validation: Signal catalog validated, cross-validation with exit codes
- ✅ Lint: All languages passed (Biome, Ruff, golangci-lint, goneat)
- ✅ Format: All files formatted (goneat, Biome)
- ✅ Typecheck: TypeScript passed
- ✅ Sync: All assets synced to language wrappers
- ✅ Signal fixtures: Valid fixtures parse correctly, invalid fixtures rejected
...
- ✅ Signal catalog with 8 signals and 6 behaviors (SSOT established)
- ✅ JSON Schema validation for signal configurations
- ✅ Comprehensive module standard with Windows strategy
- ✅ HTTP `/admin/signal` minimum spec (interim)
- ✅ Exit code corrections (SIGUSR1/USR2)
- ✅ Signal fixtures for testing and validation
- ✅ Windows fallback strategy documentation
- ✅ Helper library API requirements (with feedback integration)
- ✅ Sync configuration for signal assets to language wrappers
- ✅ CI validation for signal catalog

**v0.1.x Helper Libraries (Immediate Next)**:

- 🔜 **gofulmen**: Signal handling module implementation
- 🔜 **pyfulmen**: Signal handling module implementation
- 🔜 **tsfulmen**: Signal handling module implementation
...
- 🔜 **groningen** (TypeScript workhorse): Adopt signal handling for graceful shutdown
- 🔜 **percheron** (Python workhorse): Adopt signal handling for graceful shutdown
- 🔜 Template integration: CDRL workflow hooks for signal handling
