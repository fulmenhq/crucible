...
- ✅ Structured credential objects replace flat string values
- ✅ Smart masking by credential type (api_key, password, token)
...
- ✅ Domain consistency fix (schemas.fulmenhq.dev)
...
**Breaking Change**: Renamed `secrets` → `credentials`
...
### For fulmen-secrets Tool (v0.1.0 → v0.1.1+)
...
- ❌ "Reference-based secrets deferred to v1.1.0"
- ❌ "Expiry/rotation metadata deferred to v1.1.0"

**Actual v1.0.0 Features**:

- ✅ Structured credential objects with types
- ✅ Smart masking, lifecycle metadata, rotation policies
- ✅ External references (`ref` field) already available
- ✅ All enterprise features included in v1.0.0
...
1. ✅ Schema shape correct (credential objects, oneOf enforcement, metadata)
2. ✅ Domain consistency fixed (schemas.fulmenhq.dev)
3. ✅ Example numbering fixed (no duplicates)
4. ✅ Documentation updated (all legacy `secrets:` examples converted to `credentials:`)
5. ✅ Schema Evolution section corrected (accurate v1.0.0 feature list)
...
1. ✅ Crucible v0.2.17 published
2. ⏳ gofulmen embed refresh (schema updates)
3. ⏳ fulmen-secrets v0.1.1 development continues with schema support
4. ⏳ Drop local schema copies from fulmen-secrets (use gofulmen embed)
