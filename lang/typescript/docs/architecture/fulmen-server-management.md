...
Templates should expose standard targets for server orchestration:

### Required Targets

```makefile
# Start server in given configuration class
.PHONY: server-start-%
server-start-%:
	@echo "Starting server in $* configuration..."
	# Implementation: resolve port, start server, wait for health check
	# Exit with appropriate code on failure

# Stop server for given configuration class
.PHONY: server-stop-%
server-stop-%:
	@echo "Stopping server in $* configuration..."
	# Implementation: read PID from pidFile, send SIGTERM, cleanup

# Check server status
.PHONY: server-status-%
server-status-%:
	@echo "Checking server status for $*..."
	# Implementation: check PID, verify health endpoint
